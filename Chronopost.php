...
class Chronopost
{
    ...
    public static $tracking = 'https://www.chronopost.fr/tracking-no-cms/suivi-page?listeNumerosLT=' ;
    ...
    public function wrapper($payloads){
        ...
        foreach ($payloads['customer'] as $k=>$v){
            $shipp->customerValue->{"customer".$k} = $v ;
        }

        $shipp->customerValue->printAsSender ='N' ;

        foreach ($payloads['recipient'] as $k=>$v){
            $shipp->recipientValue->{"recipient".$k} = $v ;
        }

        foreach ($payloads['ref'] as $k=>$v){
            $shipp->refValue->{$k} = $v ;
        }
...
class CustomerValue
{
    ...
    public $customerCivility = 1;       // string
    public $customerContactName;    // string
    public $customerCountry = 'FR';        // string
    public $customerCountryName = 'FRANCE';    // string
...
class SkybillValue
{
    ...
    /**
     * Code Produit Chronopost
     * <li>0 : Chrono Retrait Bureau</li>
     * <li>01 : Chrono 13</li>
     * <li>86 : Chrono Relais</li>
     * cf ANNEXE 8 : CODES PRODUITS ACCEPTES PAR LE WS SHIPPING
     * @var string
     */
    public $productCode;
